...
def compile_dossier(
    ...
    # ---- Detect case type and branch ----
    case_type = case_data.get('case_type', '')
    is_disability = (
        case_type == 'disability_claim'
        or 'claimant' in str(case_data.get('subject', '')).lower()
        or 'timeline' in case_data
        or 'medical_summary' in case_data
        or 'prior_claims' in case_data
        or any('DC-' in str(e) for e in case_data.get('entities', []))
    )

    if is_disability:
        return _compile_disability_dossier(case_data)

    # Extract data (provider fraud path)
...
def _compile_disability_dossier(case_data: Dict) -> str:
    """Generate dossier markdown for a disability claim investigation."""
    subject = case_data.get('subject', 'Unknown Claimant')
    scheme = case_data.get('scheme', case_data.get('hypothesis', 'Suspicious Disability Claim'))
    evidence = case_data.get('evidence', [])
    timeline = case_data.get('timeline', [])
    medical_summary = case_data.get('medical_summary', {})
    inconsistencies = case_data.get('inconsistencies', [])
    prior_claims = case_data.get('prior_claims', [])
    policy_info = case_data.get('policy_details', {})
    recommendation = case_data.get('recommendation', '')

    dossier = f"""# DISABILITY CLAIM INVESTIGATION DOSSIER
...
This investigation examined a disability claim filed by {subject} flagged for {scheme}. The investigation identified {len(evidence)} evidence points and {len(inconsistencies)} inconsistencies warranting further review.
...
"""
    if timeline:
        dossier += "| Date | Event | Details |\n|------|-------|---------|\n"
        for evt in timeline:
            if isinstance(evt, dict):
                dossier += f"| {evt.get('date', 'N/A')} | {evt.get('event_type', 'N/A')} | {evt.get('description', '')} |\n"
            else:
                dossier += f"| — | — | {evt} |\n"
    else:
        dossier += "_No structured timeline data provided._\n"

    dossier += "\n---\n\n## MEDICAL RECORDS ANALYSIS\n\n"
    if medical_summary:
        if isinstance(medical_summary, dict):
            dossier += f"- **Treating Providers**: {len(medical_summary.get('treating_providers', []))}\n"
            dossier += f"- **Supports Claimed Disability**: {medical_summary.get('supports_claimed_disability', 'Unknown')}\n"
            dossier += f"- **Confidence**: {medical_summary.get('confidence', 'Unknown')}\n\n"
            if medical_summary.get('documentation_summary'):
                dossier += f"**Summary**: {medical_summary['documentation_summary']}\n\n"
            if medical_summary.get('documentation_gaps'):
                dossier += "**Documentation Gaps**:\n"
                for gap in medical_summary['documentation_gaps']:
                    dossier += f"- {gap}\n"
            if medical_summary.get('consistency_flags'):
                dossier += "\n**Consistency Flags**:\n"
                for flag in medical_summary['consistency_flags']:
                    dossier += f"- ⚠️ {flag}\n"
        else:
            dossier += f"{medical_summary}\n"
    else:
        dossier += "_No medical summary available._\n"

    dossier += "\n---\n\n## POLICY ANALYSIS\n\n"
    if policy_info and isinstance(policy_info, dict):
        dossier += f"- **Policy ID**: {policy_info.get('policy_id', 'N/A')}\n"
        dossier += f"- **Policy Age (months)**: {policy_info.get('policy_age_months', 'N/A')}\n"
        dossier += f"- **Monthly Benefit**: ${policy_info.get('monthly_benefit', 0):,.2f}\n"
        dossier += f"- **Occupation**: {policy_info.get('occupation', 'N/A')}\n"
        if policy_info.get('risk_flags'):
            dossier += "\n**Risk Flags**:\n"
            for rf in policy_info['risk_flags']:
                dossier += f"- ⚠️ {rf}\n"
    else:
        dossier += "_No policy details provided._\n"

    dossier += "\n---\n\n## INCONSISTENCIES & RED FLAGS\n\n"
    if inconsistencies:
        for incon in inconsistencies:
            if isinstance(incon, dict):
                severity = incon.get('severity', 'unknown').upper()
                dossier += f"### [{severity}] {incon.get('type', 'Unknown')}\n\n"
                dossier += f"{incon.get('description', '')}\n\n"
                if incon.get('evidence'):
                    dossier += f"**Evidence**: {incon['evidence']}\n\n"
            else:
                dossier += f"- {incon}\n"
    else:
        dossier += "_No inconsistencies identified._\n"

    dossier += "\n---\n\n## PRIOR CLAIM HISTORY\n\n"
    if prior_claims:
        dossier += "| Carrier | Year | Diagnosis | Duration | Outcome |\n"
        dossier += "|---------|------|-----------|----------|--------|\n"
        for pc in prior_claims:
            if isinstance(pc, dict):
                dossier += (
                    f"| {pc.get('carrier', 'N/A')} | {pc.get('year', 'N/A')} "
                    f"| {pc.get('diagnosis', 'N/A')} | {pc.get('duration_months', 'N/A')} mo "
                    f"| {pc.get('outcome', 'N/A')} |\n"
                )
            else:
                dossier += f"| — | — | {pc} | — | — |\n"
    else:
        dossier += "_No prior claims on record._\n"
    ...
    dossier += "\n---\n\n## RECOMMENDATION\n\n"
    if recommendation:
        dossier += f"{recommendation}\n"
    else:
        dossier += "1. **Review**: Escalate for senior analyst review\n"
        dossier += "2. **IME**: Order independent medical examination if not already done\n"
        dossier += "3. **Surveillance**: Consider field investigation if activity inconsistency found\n"
        dossier += "4. **Provider Review**: Flag treating provider if pattern detected\n"

    dossier += "\n---\n\n"
    dossier += "*This dossier was generated by the Claims Investigation Copilot AI system.*\n"

    _tool_log('compile_dossier', f'Disability dossier generated ({len(dossier)} chars)')
    return dossier
